...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=(480,640), scale = 1, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        super().__init__()
        # img_size = to_2tuple(img_size)
        img_size = tuple(map(lambda x: x//scale, img_size))
...
class RGBXTransformer(nn.Module):
    def __init__(self, img_size=(480, 640), patch_size=16, rgb_chans=1, x_chans=3,num_classes=1000, embed_dims=[64, 128, 256, 512], 
        ...
        # patch_embed
        self.patch_embed1 = OverlapPatchEmbed(img_size=img_size, scale = 1, patch_size=7, stride=4, in_chans=rgb_chans,
                                              embed_dim=embed_dims[0])
        self.patch_embed2 = OverlapPatchEmbed(img_size=img_size, scale = 4, patch_size=3, stride=2, in_chans=embed_dims[0],
        ...
        self.extra_patch_embed1 = OverlapPatchEmbed(img_size=img_size, scale = 1, patch_size=7, stride=4, in_chans=x_chans,
                                              embed_dim=embed_dims[0])
        self.extra_patch_embed2 = OverlapPatchEmbed(img_size=img_size, scale = 4, patch_size=3, stride=2, in_chans=embed_dims[0],
                                              embed_dim=embed_dims[1])
    ...
    def forward_features(self, x_rgb, x_e):
        ...
        #preprocess the depth image to HHA format
...
class mit_b4(RGBXTransformer):
    def __init__(self, fuse_cfg=None, **kwargs):
        super(mit_b4, self).__init__(
            patch_size=4, embed_dims=[128, 256, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
