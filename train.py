...
from pytorch_lightning.callbacks import ModelCheckpoint, LearningRateMonitor, EarlyStopping, GPUStatsMonitor
...
def main():
    ...
    config.TRAINER.SCALING = _scaling**0.5
    config.TRAINER.TRUE_LR = config.TRAINER.CANONICAL_LR * config.TRAINER.SCALING
    # config.TRAINER.WARMUP_STEP = math.floor(config.TRAINER.WARMUP_STEP / _scaling)
    
    config.DATASET.CROSS_MODAL = True if config.LOFTR.BACKBONE_TYPE == 'CrossModal' else False
